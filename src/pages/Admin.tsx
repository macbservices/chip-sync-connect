...
import {
  Smartphone, ArrowLeft, Plus, Check, X, Package, DollarSign,
  Wallet, Pencil, Trash2, RefreshCw, Users
} from "lucide-react";
...
const Admin = () => {
  ...
  const fetchAll = async () => {
    ...
    const [{ data: svcData }, { data: ordData }, { data: rechData }, { data: chipData }] = await Promise.all([
      ...
      supabase.from("chips").select("id, phone_number, operator, status").eq("status", "active"),
    ]);
  ...
  const approveOrder = async () => {
    ...
    toast.success("Pedido aprovado e ativado!");
  ...
  const approveRecharge = async (r: RechargeRequest) => {
    ...
    // Credit balance to user's profile
  ...
  const rejectRecharge = async (id: string) => {
    ...
    <div className="flex min-h-screen flex-col bg-background">
        ...
        <div className="container flex h-16 items-center justify-between">
          ...
          <nav className="flex gap-1">
            <Button variant={tab === "orders" ? "default" : "ghost"} size="sm" onClick={() => setTab("orders")}>
              ...
              {pendingOrders > 0 && <Badge className="ml-1 h-5 w-5 p-0 flex items-center justify-center text-xs">{pendingOrders}</Badge>}
            </Button>
            <Button variant={tab === "recharges" ? "default" : "ghost"} size="sm" onClick={() => setTab("recharges")}>
              <Wallet className="mr-1 h-4 w-4" /> Recargas
              {pendingRecharges > 0 && <Badge className="ml-1 h-5 w-5 p-0 flex items-center justify-center text-xs">{pendingRecharges}</Badge>}
            </Button>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Gerencie os serviços disponíveis na loja</CardDescription>
              </div>
                      ...
                      <TableRow key={svc.id}>
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            <Button variant="ghost" size="icon" onClick={() => openServiceDialog(svc)}>
                              <Pencil className="h-4 w-4" />
                            </Button>
                            <Button variant="ghost" size="icon" onClick={() => toggleService(svc.id, svc.is_active)}>
        ...
        {tab === "orders" && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Aprove pedidos e atribua chips/números</CardDescription>
            </CardHeader>
            <CardContent>
              {orders.length === 0 ? (
                          ...
                          <div className="flex gap-1">
                            {order.status === "pending_payment" && (
                              <>
                                ...
                                <Button variant="ghost" size="icon" title="Cancelar" onClick={() => cancelOrder(order.id)}>
        ...
        {tab === "recharges" && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>Confirme pagamentos PIX e credite o saldo dos clientes</CardDescription>
            </CardHeader>
            <CardContent>
              {recharges.length === 0 ? (
                    ...
                    {recharges.map((r) => (
                      <TableRow key={r.id}>
                        ...
                        <TableCell>
                          {r.status === "pending" && (
                            <div className="flex gap-1">
                              <Button variant="ghost" size="icon" title="Aprovar" onClick={() => approveRecharge(r)}>
                                <Check className="h-4 w-4 text-success" />
                              </Button>
                              <Button variant="ghost" size="icon" title="Rejeitar" onClick={() => rejectRecharge(r.id)}>
                                <X className="h-4 w-4 text-destructive" />
                              </Button>
                            </div>
                          )}
      ...
      <Dialog open={orderDialogOpen} onOpenChange={setOrderDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>Chip disponível (atribuir automaticamente)</Label>
              <Select value={orderChipId} onValueChange={setOrderChipId}>
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <Textarea value={orderNotes} onChange={(e) => setOrderNotes(e.target.value)} placeholder="Observações" />
            </div>
            <Button onClick={approveOrder} className="w-full">
              <Check className="mr-2 h-4 w-4" /> Confirmar e Ativar
            </Button>
          </div>
        </DialogContent>
      </Dialog>
