-- Expand payment_gateway_settings to describe every gateway's credential form
-- generically (credential_schema), and seed the market gateways requested:
-- Efí (kept active/default), Mercado Pago, Asaas, Stripe, PagBank, PicPay and
-- a formal "PIX Manual/Direto" entry.
--
-- NOTE: the live checkout in src/pages/Recharge.tsx today does its own static
-- PIX-key + AI-verified-proof flow and does not call any edge function that
-- reads this table yet. Marking 'efi' as active here (as requested) does not
-- change that live flow by itself — see supabase/functions/process-payment,
-- which is the new opt-in router other code can be pointed at later.

ALTER TABLE public.payment_gateway_settings
  ADD COLUMN IF NOT EXISTS credential_schema jsonb NOT NULL DEFAULT '[]'::jsonb;

-- recharge_requests currently tracks the external PIX transaction id by
-- string-matching a "txid: ..." prefix inside admin_notes (see efi-pix).
-- New gateways get a proper dedicated column instead; efi-pix is left as-is.
...
-- Guarantee exactly Efí is active, regardless of what existed before.
UPDATE public.payment_gateway_settings SET is_active = (gateway = 'efi');

-- admin_list_payment_gateways now derives configured_fields generically from
-- credential_schema instead of the old Efí-only hardcoded 4 fields, so it
-- works for every gateway without further migrations.
